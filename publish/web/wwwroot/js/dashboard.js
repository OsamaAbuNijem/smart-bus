...
async function saveStudent() {
  ...
  let res;
  if (editingId) {
    res = await apiPost(`/students/${editingId}`, body); // PUT not via proxy easily; simplified
  } else {
...
async function saveTrip() {
  ...
  if (!name || !time) { alert('الرجاء تعبئة اسم الرحلة والوقت'); return; }

  const body = {
    name,
    type: parseInt(document.getElementById('tf-type').value),
    scheduledDeparture: new Date().toISOString().split('T')[0] + 'T' + time + ':00',
    repeatDays: parseInt(document.getElementById('tf-repeat').value)
  };

  const busId = document.getElementById('tf-bus').value;
  if (busId) body.busId = busId;

  const res = await apiPost('/trips', body);
  if (res?.ok) {
...
async function saveBus() {
  ...
  const res = await apiPost('/buses', body);
  if (res?.ok) {
...
function fillTripForm(d) {
  ...
  document.getElementById('tf-name').value = d?.name || d?.routeName || '';
